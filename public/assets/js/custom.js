...
function add_loading($form_id) {
    ...
    var submit_button = document.querySelector('#' + $form_id + ' button[type="submit"]:last-child');
    if (submit_button) {
            ...
            success: function ($response) {
                ...
                if ($response.data) {
                    if ($response.data.operator) {
                        modalId = editOperator($response.data.operator, $updateUrl,$mainModalId);
                    }
...
function editOperator($edit_data, $updateUrl, $mainModal)
{
    var cloned_modal = $(`#${$mainModal}`).clone();
    var $title_msg = __('Update Operator');
    resetModal(cloned_modal,$updateUrl,$title_msg,$mainModal,updateOperatorDTRow);

    var Disabled = true;
    cloned_modal.find('.name').val($edit_data.name).attr('disabled', Disabled).css({'color':'#A59123'});
    cloned_modal.find('.employee_id').val($edit_data.employee_id).attr('disabled', Disabled).css({'color':'#A59123'});
    cloned_modal.find('.operator_type_id').val($edit_data.operator_type_id).addClass('select_local');
    if ($edit_data.status === 1) {
        cloned_modal.find('#status').prop('checked', true);
    } else {
        cloned_modal.find('#status').prop('checked', false);
    }
    return 'cloned-modal-'+$mainModal;
}
...
function updateOperatorDTRow($response) {
    var $targetTable = $(datatableID);
    let extract_data = $response.data.operator;
    let s_row = $('#tr_' + extract_data.id);
    $targetTable.dataTable().fnUpdate(extract_data.type, s_row, 2, false);
    $targetTable.dataTable().fnUpdate(extract_data.status, s_row, 4, false);
    hideModal();
    notify($response.message, 'success')
}
...
function editStation($edit_data, $updateUrl, $mainModal)
{
    var cloned_modal = $(`#${$mainModal}`).clone();
    var $title_msg = __('Update Station');
    resetModal(cloned_modal,$updateUrl,$title_msg,$mainModal,updateStationDTRow);

    cloned_modal.find('#name').val($edit_data.name);
    cloned_modal.find('#max_order_count').val($edit_data.max_order_count);
    cloned_modal.find('#warehouse_id').val($edit_data.warehouse_id).attr('disabled',true);
    if ($edit_data.status === 1) {
        cloned_modal.find('#status').prop('checked', true);
    } else {
        cloned_modal.find('#status').prop('checked', false);
...
function updateStationDTRow($response) {
    var $targetTable = $(datatableID);
    let extract_data = $response.data.station;
    let s_row = $('#tr_' + extract_data.id);
    $targetTable.dataTable().fnUpdate(extract_data.data.name, s_row, 1, false);
    $targetTable.dataTable().fnUpdate(extract_data.data.max_order_count, s_row, 2, false);
    $targetTable.dataTable().fnUpdate(extract_data.status, s_row, 3, false);
    hideModal();
    notify($response.message, 'success')
}
