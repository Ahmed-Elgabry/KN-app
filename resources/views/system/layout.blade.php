...
<script>
        ...
        if(lang_id == 2){
         ...
         $('.'+className).summernote({
           callbacks: {
               ...
               //     const processedHtml = pastedHtml.replace(fontFamilyRegex, '');
               //
               //     // Set the modified HTML back into the editor
               //     $('.'+className).summernote('code', processedHtml);
               // }
           },
    ...
    $('.modal').on('hide.bs.modal', function (event) {
        ...
        let modalTitle = $('#main_title');

        // Define common actions for multiple modals
        if (elementId === 'attribute-group-modal' || elementId === 'attribute-modal' || elementId === 'child-keyword-modal'|| elementId === 'child-keyword-form' || elementId ==='keyword-modal') {
            $("#main-form").trigger('reset');
            $('.invalid-feedback').css('display', 'none');
            $('#save_btn_edit').text(__('Save'));
        }
...
<script>
    ...
    function QuickSearch(){
        ...
        switch ($('input[name="quick_search_type"]:checked').val()) {
            case 'user':
                var name_key = 'user_full_name';
                break;
            case 'customer':
                var name_key = 'name';
                break;
            case 'manufacturer':
                var name_key = 'manufacturer_title';
                break;
