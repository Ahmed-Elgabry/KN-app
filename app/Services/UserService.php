...
use App\Filters\DepartmentId;
use App\Filters\Email;
use App\Filters\FullName;
use App\Filters\Id;
use App\Filters\Mobile;
use App\Filters\Name;
use App\Filters\PermissionGroupId;
use App\Filters\Status;
use App\Filters\Telephone;
use App\Filters\UserFullName;
...
{
    protected $user_repository, $activity_log_repository, $permission_group_repository, $activity_log_service, $auth_session_service, $department_repository;

    public function __construct(UserRepository $user_repository, ActivityLogRepository $activity_log_repository, PermissionGroupRepository $permission_group_repository, AuthSessionService $auth_session_service, ActivityLogService $activity_log_service, DepartmentRepository $department_repository)
    {
        parent::__construct();
    ...
    public function findById($id)
    {
        ...
        $datatablesData = [
            ...
            'Departments' => array_column($this->department_repository->get()->toArray(), 'name', 'id'),
        ];
    ...
    public function loadViewData(): array
    {
        ...
        $this->otherData(['Departments' => (new DepartmentService($this->department_repository))->departmentArray()]);
    ...
    public function create(): array
    {
        ...
        $this->breadcrumb('User', 'system.user.index');
        $this->otherData([
            'departments' => (new DepartmentService($this->department_repository))->departmentArray(),
